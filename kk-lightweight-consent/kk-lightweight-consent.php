...
class Lightweight_Consent_Mode {
	...
	public function render_admin_page() {
		...
		<h2>GTM / Consent Mode</h2><table class="form-table"><tr><th>GTM Container ID</th><td><input name="<?php echo esc_attr(self::OPTION_KEY); ?>[gtm_container_id]" value="<?php echo esc_attr($o['gtm_container_id']); ?>"></td></tr><tr><th>Inject GTM snippet</th><td><input type="checkbox" name="<?php echo esc_attr(self::OPTION_KEY); ?>[gtm_inject]" value="1" <?php checked($o['gtm_inject'],1); ?>></td></tr><tr><th>Default analytics checked</th><td><input type="checkbox" name="<?php echo esc_attr(self::OPTION_KEY); ?>[default_analytics]" value="1" <?php checked($o['default_analytics'],1); ?>></td></tr><tr><th>Default marketing checked</th><td><input type="checkbox" name="<?php echo esc_attr(self::OPTION_KEY); ?>[default_marketing]" value="1" <?php checked($o['default_marketing'],1); ?>></td></tr><tr><th>Default personalization checked</th><td><input type="checkbox" name="<?php echo esc_attr(self::OPTION_KEY); ?>[default_personalization]" value="1" <?php checked($o['default_personalization'],1); ?>></td></tr></table><details style="margin-top:10px;"><summary><strong>Google Tag Manager setup guide</strong></summary>
			<p><strong>Plugin-side setup</strong>: Enter your GTM Container ID (for example GTM-XXXXXXX). Enable GTM injection only if GTM is not already installed by your theme or another plugin. Save settings and clear cache.</p>
			<p><strong>Consent Mode behavior</strong>: This plugin sends default consent before user choice, then updated consent after accept/reject/save. Consent Mode v2 types used: analytics_storage, ad_storage, ad_user_data, ad_personalization, functionality_storage, personalization_storage, security_storage.</p>
			<p><strong>GTM Consent Overview</strong>: In GTM go to Admin → Container Settings → Additional Settings and enable Consent Overview. Then open Tags and review consent settings.</p>
			<p><strong>Recommended tag settings</strong>: GA4 should require analytics_storage. Ads/remarketing should require ad_storage, ad_user_data, and ad_personalization where applicable. Do not fire marketing tags before consent.</p>
			<p><strong>Testing in GTM Preview</strong>: Use an incognito session. Verify kk_consent_default before interaction, then kk_consent_update after Accept all, Reject all, and Save choices scenarios.</p>
			<p><strong>Browser console</strong>: Run <code>window.dataLayer</code> and check kk_consent_default / kk_consent_update events and consent state changes.</p>
			<ul><li>Do not install the same GTM container twice.</li><li>Do not fire GA4/Ads before consent in basic consent mode.</li><li>Do not rely only on the visual banner; verify in GTM Preview.</li><li>Publish GTM container changes.</li><li>Clear cache after plugin or GTM updates.</li></ul>
			<p><strong>Debug mode</strong>: Enable only during testing. Disable in production after verification.</p>
		</details><?php submit_button(); ?></form></div><?php
	}
}
