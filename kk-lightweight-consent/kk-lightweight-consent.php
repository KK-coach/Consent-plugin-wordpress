...
class KK_Lightweight_Consent {
	...
	private function defaults() {
		return array(
			...
			'gtm_container_id'     => '',
			'gtm_inject'           => 0,
			'banner_text_hu'       => 'Sütiket és hasonló technológiákat használunk az oldal működtetéséhez, analitikához és marketing méréshez. A szükséges sütik mindig aktívak. Az analitikai és marketing célú mérések csak hozzájárulás után indulnak.',
			'banner_text_en'       => 'We use cookies and similar technologies to operate this website, analytics, and marketing measurement. Necessary cookies are always active. Analytics and marketing measurement starts only after consent.',
			'policy_url'           => '',
			'cookie_days'          => 180,
	...
	public function enqueue_assets() {
		...
		wp_enqueue_style(
			...
			plugin_dir_url( __FILE__ ) . 'assets/kk-consent.css',
			array(),
			'1.0.0'
		);
		...
		wp_enqueue_script(
			'kk-lightweight-consent',
			plugin_dir_url( __FILE__ ) . 'assets/kk-consent.js',
			array(),
			'1.0.0',
			true
		);
		...
		$config    = array(
			...
			'labels'         => array(
				...
				'reject'         => $options[ 'label_reject_' . $lang ],
	...
	public function print_consent_defaults() {
		...
		?>
		<script id="kk-consent-default">
		window.dataLayer = window.dataLayer || [];
		function gtag(){dataLayer.push(arguments);}
		(function(){
			var storageKey = <?php echo wp_json_encode( $store_key ); ?>;
			var debug = <?php echo esc_js( $debug ); ?>;
			var saved = null;
			try {
			...
			var analyticsGranted = !!(parsed && parsed.choices && parsed.choices.analytics);
			var marketingGranted = !!(parsed && parsed.choices && parsed.choices.marketing);
			...
			var defaultEvent = {
				event: 'kk_consent_default',
				kk_consent_status: parsed ? 'saved' : 'unset'
			};
			dataLayer.push(defaultEvent);
			if (debug) {
				console.log('[KK Consent] default payload', payload);
				console.log('[KK Consent] default event', defaultEvent);
			}
		})();
		</script>
		<?php
	...
	public function render_banner_markup() {
		...
		$lang = $this->current_lang();
		$text = $options[ 'banner_text_' . $lang ];
		?>
		<div id="kk-consent-root" class="kk-consent-root" data-lang="<?php echo esc_attr( $lang ); ?>">
			<div class="kk-consent-banner" role="dialog" aria-live="polite" aria-label="Cookie consent">
				<p class="kk-consent-text"><?php echo wp_kses_post( $text ); ?></p>
				<a class="kk-consent-policy" href="#" hidden></a>
				<div class="kk-consent-actions">
					<button type="button" class="kk-btn kk-reject"></button>
					<button type="button" class="kk-btn kk-settings"></button>
					<button type="button" class="kk-btn kk-accept"></button>
				</div>
				<div class="kk-consent-panel" hidden>
					...
					<button type="button" class="kk-btn kk-save"></button>
				</div>
			</div>
			<button type="button" class="kk-consent-reopen" hidden></button>
		</div>
		<?php
	}
	...
	public function admin_menu() {
		add_options_page(
			esc_html__( 'KK Consent', 'kk-lightweight-consent' ),
			esc_html__( 'KK Consent', 'kk-lightweight-consent' ),
			'manage_options',
			'kk-lightweight-consent',
	...
	public function render_admin_page() {
		...
		?>
		<div class="wrap">
			<h1><?php echo esc_html__( 'KK Lightweight Consent Settings', 'kk-lightweight-consent' ); ?></h1>
			<form method="post" action="options.php">
				<?php settings_fields( 'kk_lwc_settings_group' ); ?>
				<table class="form-table" role="presentation">
					...
					<tr><th scope="row">GTM Container ID</th><td><input type="text" name="<?php echo esc_attr( self::OPTION_KEY ); ?>[gtm_container_id]" value="<?php echo esc_attr( $options['gtm_container_id'] ); ?>" class="regular-text" placeholder="GTM-XXXXXXX"></td></tr>
					...
					<tr><th scope="row">Privacy/cookie policy URL</th><td><input type="url" name="<?php echo esc_attr( self::OPTION_KEY ); ?>[policy_url]" value="<?php echo esc_attr( $options['policy_url'] ); ?>" class="regular-text"></td></tr>
